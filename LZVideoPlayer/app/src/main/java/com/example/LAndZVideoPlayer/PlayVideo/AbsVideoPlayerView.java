    ...
    protected void onPlayStateChanged(int state) {
        ...
        switch (state) {
            case LZState.STATE_NORMAL:
                ...
                break;
            case LZState.STATE_LOADING:case LZState.STATE_PLAYING_BUFFERING_START:
                break;
            case LZState.STATE_PLAYING:
   ...
   //播放时，控制条3秒自动隐藏任务
    public class DismissControllerViewTimerTask extends TimerTask {
        ...
        public void run() {
            ...
            if (state != LZState.STATE_NORMAL
                    && state != LZState.STATE_ERROR
